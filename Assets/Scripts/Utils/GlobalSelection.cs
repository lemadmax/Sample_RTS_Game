 ...
 *
 * ------------------ GlobalSelection --------------------
 * Handle player click selection and drag selection.
 *
 *********************************************************/
public class GlobalSelection : MonoBehaviour
{
    public SelectDict selectTable = new SelectDict(); // Store the selected units or building.
    ...
    void Update()
    {
        ...
        if(Input.GetMouseButtonUp(0))
        {
            if(!dragSelect)
            {
                ...
                {
                    if(hitInfo.transform.gameObject.tag == "UnitBody" && hitInfo.transform.gameObject.GetComponentInParent<UnitManager>().GetPlayerId() == gameManager.id)
                    {
                        if (Input.GetKey(KeyCode.LeftShift))
                        {
                            selectTable.Select(hitInfo.transform.parent.gameObject, SelectType.Unit);
                        }
                        else
                        {
                    ...
                    else if (hitInfo.transform.gameObject.tag == "BuildingBody" && hitInfo.transform.gameObject.GetComponentInParent<BuildingManager>().side == gameManager.side)
                    {
                        if (Input.GetKey(KeyCode.LeftShift))
                        {
                            selectTable.Select(hitInfo.transform.parent.gameObject, SelectType.Building);
                        }
                        else
                        {
                            selectTable.DeselectAll();
    ...
    {
        if (other.gameObject.tag == "UnitBody" && other.gameObject.GetComponentInParent<UnitManager>().GetPlayerId() == gameManager.id)
        {
            selectTable.Select(other.transform.parent.gameObject, SelectType.Unit);
        }
    }
}
